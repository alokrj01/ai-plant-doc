  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <p className="font-display text-lg font-medium text-green-800 animate-pulse">AI is analyzing plant data...</p>
      </div>
    );
  }
  ...
  if (!results) {
    return (
      <div className="flex flex-col items-center justify-center text-center text-gray-400 py-12 px-4 border-2 border-dashed border-gray-200 rounded-3xl bg-gray-50/50">
        ...
        <p className="font-display text-gray-500 font-medium">No predictions yet.</p>
        <p className="font-sans text-sm">Use text or image prediction above to get started.</p>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm font-bold tracking-wider uppercase text-gray-500">Official Diagnosis</span>
              </div>
              <h2 className="font-display text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight leading-tight">
                {results.disease}
              </h2>
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                {isHealthy && (
                  <Badge variant="success" className="font-sans px-3 py-1 text-sm shadow-sm">
                    Perfectly Healthy
                  </Badge>
                )}
              </div>
            ...
            <div className="w-full md:w-auto bg-white/90 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/50 min-w-[220px]">
              <div className="flex justify-between items-end mb-3">
                <span className="font-sans text-sm font-semibold text-gray-500">AI Confidence</span>
                <span className={`font-mono text-3xl font-black leading-none ${confidenceValue >= 85 ? 'text-green-600' : confidenceValue >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                  {results.confidence}
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2 shadow-md hover:shadow-lg transition-shadow duration-300">
          <CardHeader className="bg-gray-50/50 border-b border-gray-100 pb-4">
            <CardTitle className="font-display text-lg flex items-center gap-2 text-gray-800">
              <ShieldAlert className="w-5 h-5 text-blue-600" />
              About the Condition
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <p className="font-sans text-gray-700 leading-relaxed text-[15px] sm:text-base">
        ...
        <Card className="shadow-md bg-slate-50 border-slate-200 hover:shadow-lg transition-shadow duration-300">
          <CardHeader className="pb-3 border-b border-slate-200">
            <CardTitle className="font-display text-base flex items-center gap-2 text-slate-700">
          ...
          <CardContent className="pt-4 text-sm">
            {results.type === 'text' ? (
              <div className="space-y-3">
                <div>
                  <span className="font-display block text-slate-500 font-medium text-xs uppercase mb-1">Plant Type</span>
                  <span className="font-sans text-slate-800 font-semibold">{results.input.plantType}</span>
                </div>
                <div>
                  <span className="block text-slate-500 font-medium text-xs uppercase mb-1">Symptoms</span>
                  <span className="font-sans text-slate-800">{results.input.symptoms}</span>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center text-slate-600">
                  <span className="font-sans truncate max-w-[150px]" title={results.input.fileName}>{results.input.fileName}</span>
                  <Badge variant="default" className="font-mono bg-slate-200 text-slate-700 border-none">
      ...
      {!isHealthy && results.treatment && (
        <Card className="shadow-lg border-green-100 overflow-hidden">
          <CardHeader className="bg-green-50/50 border-b border-green-100">
            <CardTitle className="font-display text-xl text-green-800 flex items-center gap-2">
          ...
          <CardContent className="p-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
              ...
              <div className="p-6 md:p-8 space-y-4 bg-white hover:bg-red-50/30 transition-colors duration-500">
                <h4 className="font-display font-bold text-red-700 flex items-center gap-2 text-lg">
                ...
                <ul className="space-y-3">
                  ...
                  {results.treatment?.immediate?.map((action, index) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <span className="font-sans text-gray-700 leading-relaxed text-sm">{action}</span>
                    </li>
                  )) || <li className="font-sans text-gray-400 italic text-sm">No immediate actions available.</li>}
                </ul>
              </div>
              
              {/* Long-term Prevention */}
              <div className="p-6 md:p-8 space-y-4 bg-white hover:bg-blue-50/30 transition-colors duration-500">
                <h4 className="font-display font-bold text-blue-700 flex items-center gap-2 text-lg">
                ...
                <ul className="space-y-3">
                  {results.treatment?.prevention?.map((prevention, index) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <span className="font-sans text-gray-700 leading-relaxed text-sm">{prevention}</span>
                    </li>
                  )) || <li className="font-sans text-gray-400 italic text-sm">No prevention strategies available.</li>}
                </ul>
              </div>
